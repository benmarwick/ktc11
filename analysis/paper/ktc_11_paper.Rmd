...
the_plot_ages <- Stratiplot(sediments_strat_plot[,c(-1, -2)], 
           ...
           lwd.h = 0.5,
           labelRot = 90,
           yticks =  calibrated_dates$dates_table$`Depth below surface (m)`))
...
the_plot_ages$y.scales$labels <-  
  paste0(round(calibrated_dates$dates_table$`Age in years BP` / 1000,2),
         " (",
         substr(calibrated_dates$dates_table$`Material dated`,1,1),
         ")")
```

```{r problem, fig.cap="Summary of bulk sediment analysis of samples from Khao Toh Chong. Magnetic susceptibility is reported as low frequency mass specific units 10^8^ m^3^ kg^-1^"}
